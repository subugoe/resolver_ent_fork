...
/**
 * This class is a thread which connects to a single LPIR (local resolver) and is 
 * created by the <pre>Resolver</pre> class. For every single LPIR an appropriate 
 * <pre>LocalResolverConnectorThread</pre> instance is created. In its run-method the
 * Http-request is build.<p>
 * The result from the HTTP-request is parsed (getResponse-method). As a result of the thread
 * the retrieved URLs should be retrievable by using the <pre>getResponses</pre> method.
 * 
 * @author enders
 */
public class LocalResolverConnectorThread extends Thread {
	...
	/**
	 * The <pre>run</pre> method is started as soon as the thread starts. It will can only end, if
	 * <ul>
	 * <li>the request has been carried out successfully and the answer is parsed</li>
	 * <li>an error occured - no matter, if a connection error or an XML-parsing error of the response</li>
	 * <li>The calling class (<pre>Resolver</pre> interrupts the execution; usually if a timeout is assumed.
	 * This timeout can be set using the <pre>&lt;maxThreadRuntime&gt;</pre> element in the preferences file</li>
	 * </ul>
	 */
	...
	public synchronized void run (){
		...
		try{	
		    ...
		    if (statusCode != HttpStatus.SC_OK) {
		    	...
		    	writeLog("SUBResolver: Method failed: " + method.getStatusLine());
		    	writeLog(              "for URL:"+url);
		    }
	...
	/**
	 * Retrieves the result of the connection request; if no URLs are available null is returned.
	...
	/**
	 * Parses the response and calls <pre>readPURL</pre> method, for every <pre>resolvedLPIs</pre> block in the response
	...
	private LinkedList getResponse(InputStream responseStream){
		...
		try{
			...
			if (!topmostelement.getNodeName().equals("response")){
				  writeLog("SUBResolver: ERROR: xml answer doesn't match DTD");
				  writeLog(             "for URL:"+url);
				  return null;
			}
			...
			for (int x=0;x<allchildnodes.getLength();x++){
			 ...
			 writeLog("             occured while receiving response for url:");
			 writeLog("             "+url);
			 return null;
	...
	/**
	 * Parses the DOM-tree of the response for every <pre>resolvedLPIs</pre> block. This block is interpreted and a
	 * new <pre>ResolvedURL</pre> object is created for a valid block.
	...
	private ResolvedURL readPURL(Node inNode){
		...
		if ((purl!=null)&&(resolverurl!=null)&&(service!=null)){
			...
			if (purl!=null){ru.setPurl(purl);}
			if (resolverurl!=null){ru.setUrl(resolverurl);}
			if (service!=null){ru.setService(service);}
			if (servicehome!=null){ru.setServicehome(servicehome);}
			if (access!=null){ru.setAccess(access);}
			if (version!=null){ru.setVersion(version);}
